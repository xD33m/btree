...
class BTreeNode {
	...
	add(key) {
		if (this.isLeaf()) {
			...
			let child = this.getChildforNewKey(key);
	...
	// getChildforNewKey gibt die Node zurück, wo der Key eingefügt werden soll
	getChildforNewKey(key) {
	...
	remove(key) {
		if (this.isLeaf()) {
			...
			if (keyIndex === -1) {
				...
				child = this.getChildforNewKey(key); // get one node down
